...
class PostService {
  ...
  // متدهای جدید برای ویرایش (داخل کلاس هستند)
  async checkExistAndOwnership(postId, userId) {
    if (!isValidObjectId(postId))
      throw new createHttpError.BadRequest("شناسه آگهی نامعتبر است");
    const post = await this.#model.findOne({ _id: postId, userId });
    if (!post)
      throw new createHttpError.NotFound("آگهی یافت نشد یا دسترسی ندارید");
    return post;
  }

  async update(postId, data) {
    const updated = await this.#model.findByIdAndUpdate(postId, data, {
      new: true,
    });
    if (!updated)
      throw new createHttpError.InternalServerError("خطا در ویرایش آگهی");
    return updated;
  }
}
